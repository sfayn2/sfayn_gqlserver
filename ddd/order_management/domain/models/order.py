...
class Order:
    ...
    def cancel_shipment(self, shipment_id: str):
        shipment = self.get_shipment(shipment_id)
        if shipment.shipment_status in (enums.ShipmentStatus.IN_TRANSIT, enums.ShipmentStatus.SHIPPED, enums.ShipmentStatus.DELIVERED, enums.ShipmentStatus.CANCELLED):
            raise exceptions.DomainError("Cannot cancel shipment after in_transit/shipped/delivered/cancelled")
