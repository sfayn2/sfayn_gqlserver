...
class PercentageDiscountHandler(OfferHandler):
    ...
    #apply on order
    def apply_offer(self, order: models.Order) -> str:
        if self.name == enums.OfferType.PERCENTAGE_DISCOUNT:
            total_discount = 0
            is_applied = False
...
class FreeGiftOfferHandler(OfferHandler):
    ...
    def apply_offer(self, order: models.Order) -> str:
        if self.name == enums.OfferType.FREE_GIFT:
            free_gifts = []
            currency = order.get_currency()
            is_applied = False
...
class FreeShippingOfferHandler(OfferHandler):
    ...
    def apply_offer(self, order: models.Order) -> str:
        if self.name == enums.OfferType.FREE_SHIPPING:
            is_applied = False
            currency = order.get_currency()
...
class PercentageDiscountCouponOfferHandler(OfferHandler):
    ...
    def apply_offer(self, order: models.Order) -> value_objects.Money:
        if self.name == enums.OfferType.COUPON_DISCOUNT:
            total_discount = 0
            discounted_items = []
            currency = order.get_currency()
            is_applied = False
