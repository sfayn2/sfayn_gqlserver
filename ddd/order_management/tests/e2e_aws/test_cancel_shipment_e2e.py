...
from ddd.order_management.entrypoints.graphql.mutations.cancel_shipment_mutation import CancelShipmentMutation 
...
def graphene_client(mocker, user_context_tenant1_vendor_all_perms):
    ...
    class RootTestMutation(graphene.ObjectType):
        cancel_shipment = CancelShipmentMutation.Field() 
    ...
    mocker.patch(
        'ddd.order_management.infrastructure.access_control1.DynamodbAccessControl1.get_user_context',
...
# The GraphQL mutation query updated for CancelShipment
    query = """
            mutation CancelShipment($input: CancelShipmentMutationInput!) {
