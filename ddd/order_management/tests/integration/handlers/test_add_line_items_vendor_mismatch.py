...
@pytest.fixture
def fake_product_skus_different_vendor(test_constants):
    return [dtos.ProductSkusDTO(vendor_id=test_constants["vendor2"], product_sku="sku_vendor_mismatch", order_quantity=22)]

@pytest.mark.django_db
def test_add_line_items_vendor_mismatch(
    fake_customer_details,
    fake_address,
    fake_product_skus_different_vendor,
    fake_jwt_handler, 
    fake_access_control,
    domain_clock,
    seeded_all
    ...
    command = commands.AddLineItemsCommand(
        ...
        order_id="ORD-1",
        product_skus=fake_product_skus_different_vendor
    )
    ...
    assert response.message == "Vendor mismatch between order and line item."
