...
class OrderMapper:
    ...
    def to_dto(order: models.Order) -> dtos.OrderDTO:
        response_dto = dtos.OrderDTO(
                ...
                shipping_option=asdict(order.shipping_details) if order.shipping_details else None,
                payment_option=asdict(order.payment_details) if order.payment_details else None,
