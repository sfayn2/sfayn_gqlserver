...
def handle_product_approval(command: commands.ApproveProductCommand, uow: unit_of_work.DjangoUnitOfWork):
    with uow:
        ...
        print("Handle product approval")

        return event


def log_approved_product(event: events.ProductApproved, uow: unit_of_work.DjangoUnitOfWork):
    # handle the event by logging the product approved
    print(f"Approved product: {event.product_id} - {event.name} ({event.description} {event.category})")