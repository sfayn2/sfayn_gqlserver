# Generated by Django 4.2 on 2023-04-28 14:29

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('sites', '0002_alter_domain_unique'),
    ]

    operations = [
        migrations.CreateModel(
            name='Setting',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('weight_unit', models.CharField(help_text='all products weight unit will be default in {weight_unit}', max_length=5, null=True)),
                ('dimensions_unit', models.CharField(help_text='all products weight unit will be default in {dimensions_unit}', max_length=5, null=True)),
                ('product_approval', models.IntegerField(choices=[(0, 'Pending Review'), (1, 'Approved')], default=1, help_text='when product is created it requires approval before it can be published?')),
                ('country', models.CharField(blank=True, help_text='Singapore, .. ?', max_length=25, null=True)),
                ('currency', models.CharField(blank=True, help_text='USD, SGD, ..', max_length=10, null=True)),
                ('date_created', models.DateTimeField(auto_now_add=True)),
                ('date_modified', models.DateTimeField(auto_now=True)),
                ('created_by', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='user2settings', to=settings.AUTH_USER_MODEL)),
                ('site', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to='sites.site')),
            ],
        ),
    ]
